...
class Trainer:
	def __init__(self, n_clients, learning_rate, lr_decay, batch_size, epochs, n_local_epochs, cl_percentages, 
		...
		self.sv_file = "exact_flSV_" + self.w_file + ".csv"
	...
	def SV_in_train(self):
		...
		for epoch in range(self.epochs):
			...
			start = time.time()
			SVs = self.get_SVs(ctype = 'exact', cmethod = 'subsets', selected_cls = selected_cls, V_pre = V_pre)
			stop = time.time()
	...
	def get_SVs(self, ctype = 'exact', cmethod = 'subsets', selected_cls = [], V_pre = 0):
		...
		if (ctype == 'exact') and (cmethod == 'subsets'):
			...
			SV1 = eSV.by_subsets(device, V_pre = V_pre) #in dictionary
			for ag in SV1.keys():
				SVs[ag] = SV1[ag]
